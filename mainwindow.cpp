...
void MainWindow::PushButtonBuildGraph()
{
    ...
    if (CheckInput()==true)
    {
        emit SendDataToStart(parALine->text().toInt(&ok, 10), parBLine->text().toInt(&ok, 10), parXMinLine->text().toInt(&ok, 10), parXMaxLine->text().toInt(&ok, 10));
        emit StartCalc();
    }
